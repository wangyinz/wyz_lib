...
namespace SEISPP {
	...
	{	// initialize MPI parameters.
		...
		try{
			...
			string line;
			int countme=0;
			for(;getline(poisrcf, line);countme++)
			{	
				if((countme % p)!=myrank) continue;
				//char *line=(char *)gettbl(t,i);
				...
				gp.r=r0-depth;
				//DEBUG
				//cout<<r0-depth<<" is the original depth"<<endl;
				points.push_back(gp);
				amp.push_back(ampin);
	...
	vectorcls PointSourcePSSynthetic::getENZ(Hypocenter& hypo, Geographic_point& point, const double& rlat, const double& rlon, const double& relev, const double& Pinc_theta, const double& razimuth, const double& pertamp){
		...
		if(deldeg==0){
			...
			if(enableScatterPattern) spolar*=ScatterPatternP2S(cos_sc_angle, pertamp*rhoperturb, pertamp*vsperturb, vp, vs);
			return spolar;
		...
		*/
		...
		double cos_sc_angle=dotproduct(Pin, Lp);
		if(enableScatterPattern) Spolar*= ScatterPatternP2S(cos_sc_angle, pertamp*rhoperturb, pertamp*vsperturb, vp, vs); //scalar product
	...
	double getPphasetime(double a,double b)
	{
		...
		Tbl            *times1 = 0;
		Hook           *hook = 0 ;
		ttcalc ( "tttaup", "iasp91", "P", 2, &geometry, &times1, &hook ) ;
		TTTime *atime ;
	...
	double getPnphasetime(double a,double b)
	{
		...
		Tbl            *times1 = 0;
		Hook           *hook = 0 ;
		ttcalc ( "tttaup", "iasp91", "Pn", 2, &geometry, &times1, &hook ) ;
		TTTime *atime ;
		atime = (TTTime *) gettbl(times1, 0);
		return atime->value;
	}
	double getPphaseslowness(double a,double b)
	{
		...
		Tbl            *slow1 = 0;
		Hook           *hook = 0 ;
		ucalc ( "tttaup", "iasp91", "P", 2, &geometry, &slow1, &hook);
		TTSlow *u ;
	...
	int PointSourcePSSynthetic::initPtime4event(Hypocenter& hypo){
		...
		int npoints=points.size();//local number of points.
		cout<<"I have "<<npoints<<" points."<<endl;
		Ptimearray.reserve(npoints);
		Panglearray.reserve(npoints);
		//for(int i=myrank;i<npoints;i+=p){
                for(int i=0;i<npoints;i++){
			double p_2scatter_time;
			double pointidepth=r0_ellipse(deg(points[i].lat))-points[i].r;
			double azPin,deltap, Pinc_theta;
			//DEBUG
			//cout<<"Point is "<<points[i].r<<endl;
	...
	ThreeComponentSeismogram PointSourcePSSynthetic::Compute3C(const ThreeComponentSeismogram& parent, Hypocenter& hypo,double rlat, double rlon, double relev,string units)
	{
		...
		//MPI parameters initialized in the constructor
		//and status and tag are for MPI_Send & Recv functions.
		MPI_Status status;
		MPI_Request request;
		int tag = 1;//required in MPI send & recv
		...
		try{
			...
			//MPI: this loop calculate the amplitude & relative time for 
			// each scattering point in the local cpu memory space.
			int nlocalpoints=Ptimearray.size();
			
			for(int i=0,k=0;k<nlocalpoints;k++){
				i=k;
				//hook=0;
				double  s_time;
				...
				double pointidepth=r0_ellipse(deg(points[i].lat))-points[i].r;
				//DEBUG
				//cout<<"Point depth is "<<pointidepth<<endl;
				//cout<<"R is "<<r0_ellipse(deg(points[i].lat))<<endl;
				//cout<<"Point is "<<points[i].r<<endl;
				...
				try{
					...
					tau=Ptimearray[k]+s_time-patime;
					amplitude=tnamp.second;
					int it=result.sample_number(tau);
					if(it>=0 && it<result.ns){
						if(rotationtype){
							vectorcls spolar=getENZ(hypo, points[i], rlat, rlon, relev, Panglearray[k], azimuth, amp[i]);//the converted S wave azimuth@receiver
							spolar*=amplitude;
			...
			}//End of for npoints loop.
			//////////////////Now let's try MPI hahah:)//////////////////
			int src,dest;
			if(myrank!=0){
				dest=0;//Node 0 is the destination.
				//MPI_Recv(syndata.u.get_address(0,0), 3*nsamp, MPI_DOUBLE, kk, tag, MPI_COMM_WORLD, &status);
				MPI_Isend(result.u.get_address(0,0), 3*result.ns, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD, &request);
				MPI_Wait(&request, &status);
			}
			else{//myrank==0
				//blablabla
				int nsamp=result.ns;
				dmatrix tmpMat(3, nsamp);
				for(int i=1;i<p;i++){
					//tmpMat.zeros();
					MPI_Recv(tmpMat.get_address(0,0), 3*nsamp, MPI_DOUBLE, i, tag, MPI_COMM_WORLD, &status);
					//cout<<"result.u.size::tmpMat.size:"<<(int)result.u.columns()<<" "<< (int)tmpMat.columns()<<endl;
					result.u+=tmpMat;
					
				}
				//syndata.u is now a superposition of all waveforms from p nodes.
			}
